...
export default {
  ...
  computed: {
    ...
    parsedValue() {
      ...
      // userData might provide string / timestamp without value-format, coerce them into date (or array of date)
  ...
  methods: {
    ...
    // {parse, formatTo} String deals with userData input
    parseString(value) {
    ...
    handleKeydown(event) {
      ...
      if (keyCode === 9) {
        if (!this.ranged) {
          ...
          // userData may change focus between two input
          setTimeout(() => {
      ...
      // if userData is typing, do not let picker handle key input
      if (this.userInput) {
        event.stopPropagation();
        return;
      }
    ...
    emitChange(val) {
      // determine userData real change only
